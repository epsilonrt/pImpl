...
class PimplClass {
  public:
    ...
    */
    ...
    /**
       @brief Check if the private class is null

       This can happen when moving the object with std::move.

       @return true if the private class is null
    */
    bool isNull() const;

    /**
       @brief Clear the private class

       This can be used to reset the private class to a default state (same as after default construction)
       The default implementation does nothing, but subclasses may override it to clear or reset their
       private class (reset if isNull() returns true).
    */
    virtual void clear();
